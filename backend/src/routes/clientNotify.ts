...
function buildBookingDigestHtml(
        items: Array<{
...
export async function sendEmailToAdmin(email:string , amount: number, orderNumber : number,paymentNumber : string, name : string, arrivalTime : string , guestNumber : number , from : string , to :string ){
    try{
...
export async function sendBookingDigest() {
    ...
    const futureReservations = await db
        .select({ reservation: reservations, user: users })
        .from(reservations)
        .innerJoin(users, eq(reservations.user_id, users.id))
        .where(gte(reservations.end, now))
        .orderBy(asc(reservations.start));
